...
#import "NSString+ESEmoji.h"

@implementation NSString (ESEmoji)
...
- (BOOL)es_containEmoji
{
    ...
    for (NSUInteger i = 0; i < len; i++) {
        ...
        if ((bt | 0x0F) == 0xEF) {
            ...
            v = v << 6;
            v |= bts[i + 2] & 0x3F;
            if ([NSString es_emojiInSoftBankUnicode:v] || [NSString es_emojiInUnicode:v]) {
                return YES;
            }
            i += 2;
            continue;
        ...
        if ((bt | 0x3F) == 0xBF) {
            // 10xxxxxx  10开头,为数据字节,直接过滤
            continue;
        }
        // 不是以上情况的字符全部超过三个字节,做Emoji处理
        return YES;
    }
    return NO;
}

+ (BOOL)es_emojiInSoftBankUnicode:(short)code {
    return ((code >> 8) >= 0xE0 && (code >> 8) <= 0xE5 && (Byte)(code & 0xFF) < 0x60);
}

+ (BOOL)es_emojiInUnicode:(short)code
{
    if (code == 0x0023
        || code == 0x002A
